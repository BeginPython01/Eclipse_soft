...
**Timeline:** 10 weeks
**Version:** 1.2 — forecast horizon changed EOY → EOM; AI narrates `เหลือ` in LINE; adds daily safe-to-spend
...
- **Pillar B — Actionable forecasting:** "Projected balance on Sep 30: ฿12,400. 23% chance of shortfall before month-end."
...
- `D` = days remaining until the last day of the current calendar month
...
```
B_EOM = B_now + Σ R − ( d̃ × D × s_current )
```

`s_current` = seasonality factor for the **current month only** (horizon no longer spans multiple months).
Default `s` (Thailand): Apr = 1.15, Nov = 1.10, Dec = 1.25, else 1.00.
...
CREATE TABLE forecast_snapshots (
  ...
  projected_eom_balance DECIMAL(12,2) NOT NULL,
  daily_safe_to_spend   DECIMAL(12,2) NOT NULL,
  runway_date           DATE,
...
**T4 — `generateInsight`** (caller: S3 / S7, 🆕 also S1 for the `เหลือ` command)
...
✅ REQUIRED:  { spend_this_month: 24500, avg_3mo: 20800,
                ...
                forecast_eom: 12400, shortfall_prob: 0.23,
                daily_safe_to_spend: 1200 }
              → "Write 2 sentences in Thai. Do not invent numbers."
```

🆕 **`เหลือ` variant:** same contract, shorter prompt ("Write 1 sentence in Thai narrating the balance and forecast. Do not invent numbers.") — output length is capped tighter since it renders inline in a chat reply, not an insight card.
...
3. The dashboard displays an EOM forecast with a visible confidence level.
