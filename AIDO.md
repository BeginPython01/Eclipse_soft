...
**AI Agent Operating Manual for the จดตัง (Jodtang) project.**
...
Users log income and expenses through LINE chat (`กาแฟ 80`), an in-LINE AI chatbot for natural-language logging and Q&A, or via automatic K PLUS bank-email ingestion. A deterministic forecast engine projects the end-of-month balance, shortfall probability, and runway date.
...
| --- | --------------- | --------------------------------------------------------------------------------- |
| S1  | LINE Gateway    | ❌ (routes chatbot turns to S11 — no direct calls)                                |
| S2  | Web Frontend    | ❌                                                                                |
| S3  | Core API        | ❌                                                                                |
...
| S11 | AI Service      | ✅ (owns all AI, including the chatbot)                                           |
| S12 | LINE AI Chatbot | ⚠️ conversation layer only — no AI calls of its own, delegates entirely to S11 T5 |
...
│   │   ├── line/              (S1)
│   │   ├── line-chatbot/      (S12) ← conversation flow only, calls ai/tasks/chat.ts
│   │   ├── email/             (S8)
│   │   ├── bank-parser/       (S9)
...
Only five tasks, all inside `src/modules/ai/`:
...
| T5 `chatbotReply`         | S12 (LINE AI Chatbot) | Free-form Q&A about the user's own finances in LINE; may also route a message into T1 for logging | P1               |

`chatbotReply` (T5) narrates and routes only — it never computes a number itself. If the user asks something like "เดือนนี้เหลือเงินเท่าไหร่" it must call the existing S5 forecast/query functions to get the real figure and only use AI to phrase the Thai-language reply around that figure (same contract as T4). If the message looks like a transaction ("กาแฟ 80"), T5 hands it to T1's extraction schema rather than answering conversationally, and the result still lands as `status = 'pending'` per A3.

Every task must pass through the **Guard Layer**, in this order:

```
1. ai_enabled check   → else deterministic fallback
2. Cache lookup       → return on hit (no provider call; T5 caches per normalized intent, not full transcript)
3. Rate limit         → 20/user/day (T1, T4); 50/day system-wide (T3); 30/user/day (T5)
...
**T5-specific guardrails:**

- Conversation context sent to the provider is limited to the **last 3 turns**, never the user's full chat history or raw transaction rows (A5 still applies — only pre-computed aggregates may be included in context).
- If `AI_ENABLED=false` or the chatbot is unavailable, S12 falls back to the existing rule-based command parser (e.g. keyword commands like "สรุป", "เหลือเท่าไหร่") so basic functionality survives per A4.
- Every chatbot reply that includes a number must have that number sourced from a deterministic module call in the same request — never generated by the model — and A2/A9 (Zod validation + ✨ AI badge/disclaimer) apply to chatbot replies exactly as they do to `generateInsight`.
...
- `buffer` = user-defined minimum balance floor, default ฿0 (configurable in `/settings`)
- Reuses the exact same `B_now`, `Σ R_in`, `Σ R_out`, `D` already computed for Level 1 — no extra inputs, no extra AI calls, no extra queries.
- If the result is negative, this is an immediate over-budget warning — the user doesn't have to wait until month-end to see the shortfall.
- Recomputed on every new transaction (`D` and `B_now` both shift), same cadence as the rest of S5.

Level 1.5 lives inside the same S5 AI-Free Zone as every other forecast level (§6) — 🔴 never let AI compute or adjust this number, only narrate it (A1). It follows the same money rules as Level 1: `Decimal` arithmetic only (M1), full recompute rather than incremental patching (M3), and `is_transfer` rows excluded (M5). Bump `method_version` per rule 6 above if the formula itself ever changes (e.g. a different `buffer` default or a new term).
...
- Explicitly instruct: _"Extract only. Do not infer, estimate, or calculate any value not literally present. Return null for absent fields."_
...
### 11.6 Adding an intent to the LINE AI chatbot

```
1. Confirm the intent needs conversational handling — if it's a fixed command,
   add a keyword shortcut in S12 instead (cheaper, no AI call)
2. Define the intent's response Zod schema in src/modules/ai/tasks/chat.ts
3. If the intent needs a number (balance, runway, budget left), fetch it via
   the existing S3/S5 module functions BEFORE calling the provider — pass the
   number in, don't ask the AI to produce it
4. If the intent is transactional ("ค่ากาแฟ 80"), route to T1's schema and the
   normal pending-transaction flow — do not let T5 create records directly
5. Add the intent to the last-3-turns context budget check
6. Add a rule-based fallback path for AI_ENABLED=false
7. Add ≥ 5 example user messages (Thai, informal) to the T5 test fixtures
8. Confirm the reply carries the ✨ AI badge + disclaimer (A9) when rendered
   in any UI surface that supports rich messages
```
...
| Letting the chatbot answer "เหลือเท่าไหร่" from the model's own reasoning | Fetch the real number from S5/S3 first, AI only narrates it |
| Sending the chatbot's full LINE conversation history as context           | Send only the last 3 turns + pre-computed aggregates        |
...
| Term                  | Meaning                                                                                                                                           |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| EOM                   | End of Month — last day of the current calendar month                                                                                             |
| Runway date           | Projected date on which balance reaches zero                                                                                                      |
| Safe-to-spend         | Level 1.5 forecast output: how much the user can spend per remaining day of the month without breaching `buffer`, recomputed on every transaction |
| B_now                 | Current account balance at the moment the forecast is computed                                                                                    |
| D                     | Days remaining in the current calendar month, counted from the forecast run date                                                                  |
| buffer                | User-configurable minimum balance floor (default ฿0) that Level 1.5 will not let the daily spendable amount dip below                             |
...
| LINE AI Chatbot (S12) | Conversational layer in the LINE Bot for natural-language logging and Q&A; holds no logic of its own, delegates all AI work to S11 task T5        |
...
This system handles money. Users trust it to tell them whether they can afford rent at the end of the month. That trust is built on **predictability**, not cleverness.

Every design decision follows from one question:

> _"If this component fails or behaves unexpectedly, does the user lose money, lose trust, or lose data — silently?"_
...
_Changelog: scope changed from end-of-year to end-of-month forecasting (EOY → EOM); added S12 LINE AI Chatbot and AI task T5 `chatbotReply`; added Forecast Engine Level 1.5 (daily safe-to-spend)._
_Companion to: `SPEC.md` v1.1_
_If `SPEC.md` is updated, update this file's rule references accordingly._
